...
Webhooks ermöglichen es, externe Systeme in Echtzeit über Ereignisse in LogistikApp zu informieren. Bei jedem auslösenden Ereignis sendet LogistikApp einen HTTP POST-Request an die von Ihnen konfigurierte URL.
...
Wenn Sie ein **Geheimnis** gesetzt haben, enthält jeder Request den Header `X-LogistikApp-Signature`. Dieser ist ein HMAC-SHA256-Hash des Request-Body mit Ihrem Geheimnis:

```javascript
// Node.js Beispiel
const crypto = require('crypto');
const signature = req.headers['x-logistikapp-signature'];
...
Bei fehlgeschlagenen Requests (HTTP 4xx/5xx oder Timeout) versucht LogistikApp es erneut:
